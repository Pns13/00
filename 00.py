...
def banner():
	...
	{m}★{k}★{h}★ POPEN NOPRIANSYAH: 082311023880 {u}{h}★{k}★{m}★{u}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\tCINTO MANDI : [green]SEJAHTERA[purple] '))
...
def menu(my_name,my_id):
	...
	cetak(nel('\tSelamat Datang [green]%s[white] ANAK HARAM'%(my_name)))
	alvino_xy(f'忠➥Your Idz : '+str(my_id))
	alvino_xy(f'忠➥Your Ip  : {ip}')
	print('')
